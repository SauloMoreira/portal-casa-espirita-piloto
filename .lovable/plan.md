...
- **Fase 4 — Classificador híbrido (LLM apenas no gap):** ✅ módulo puro `whatsappClassificadorHibrido.ts` (acionamento só no gap, validação de domínio fechado, confiança mínima 0.6, fallback seguro). Inline na edge function: `classificarComLLM` (flash-lite, temp 0, max 80 tokens, timeout 8s) acionado **somente** quando `classificar` resultou em `complexo`. Saída inválida/baixa confiança não altera nada → handoff. Auditoria: `classificador_hibrido` + `confianca_classificacao` no log. 15 testes novos, 517 no total, sem regressão.
