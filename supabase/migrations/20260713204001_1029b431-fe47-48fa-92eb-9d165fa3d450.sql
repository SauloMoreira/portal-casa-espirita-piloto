...
-- SAAS-06-C1-STAB10-C1.2-A1-FIX01 — parte 2/2
-- Corrige literais de transição e adiciona confirmação real do delete Auth em
-- fn_autocadastro_marcar_resultado_falha. Reafirma permissões nas 3 RPCs alteradas.

-- 1) fn_autocadastro_marcar_auth_criado — literal específico (sem sufixo)
...
BEGIN
  ...
  IF v_row.status = 'auth_criado' AND v_row.user_id = p_user_id THEN
    ...
    IF v_rows <> 1 THEN RAISE EXCEPTION 'FALHA_ATUALIZAR_TIMESTAMP_AUTH_CRIADO'; END IF;
    RETURN QUERY SELECT 'AUTH_CRIADO_IDEMPOTENTE'::text;
    RETURN;
  END IF;

  IF v_row.status <> 'reservado' THEN
    RAISE EXCEPTION 'TRANSICAO_INVALIDA_RESERVADO_AUTH_CRIADO';
  END IF;
...
-- 2) fn_autocadastro_marcar_resultado_falha — AUTH_DELETE_NAO_CONFIRMADO + literais
CREATE OR REPLACE FUNCTION public.fn_autocadastro_marcar_resultado_falha(
  p_idempotency_key uuid, p_request_fingerprint text,
  p_request_id uuid, p_resultado text, p_auth_delete_ok boolean)
RETURNS TABLE(result_code text)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'pg_catalog','public'
...
BEGIN
  ...
  ELSIF v_row.status = 'auth_criado' THEN
    v_auth_uid := v_row.user_id;  -- captura para auditoria em rollback_falhou
    IF p_auth_delete_ok THEN
      -- Só permite auth_criado → falhou depois que a FK ON DELETE SET NULL
      -- já limpou user_id (evidência canônica do delete real no Auth).
      IF v_row.user_id IS NOT NULL THEN
        RAISE EXCEPTION 'AUTH_DELETE_NAO_CONFIRMADO';
      END IF;
      v_new_status := 'falhou';
    ELSE
  ...
  ELSE
    IF p_auth_delete_ok THEN
      RAISE EXCEPTION 'TRANSICAO_INVALIDA_AUTH_CRIADO_FALHOU';
    ELSE
      RAISE EXCEPTION 'TRANSICAO_INVALIDA_AUTH_CRIADO_ROLLBACK_FALHOU';
    END IF;
  END IF;
  ...
  GET DIAGNOSTICS v_rows = ROW_COUNT;
  IF v_rows <> 1 THEN
    IF v_origem = 'reservado' THEN
      RAISE EXCEPTION 'TRANSICAO_INVALIDA_RESERVADO_FALHOU';
    ELSIF v_new_status = 'falhou' THEN
      RAISE EXCEPTION 'TRANSICAO_INVALIDA_AUTH_CRIADO_FALHOU';
    ELSE
      RAISE EXCEPTION 'TRANSICAO_INVALIDA_AUTH_CRIADO_ROLLBACK_FALHOU';
    END IF;
...
-- 3) fn_autocadastro_assistido_publico — literal específico na guarda ROW_COUNT
...
BEGIN
  ...
  IF v_row.status <> 'auth_criado' THEN
    RAISE EXCEPTION 'TRANSICAO_INVALIDA_AUTH_CRIADO_CONCLUIDO';
  END IF;
