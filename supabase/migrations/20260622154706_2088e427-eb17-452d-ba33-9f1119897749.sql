-- =====================================================================
-- REGRA DE SINCRONIZAÇÃO (horario_previsto <-> agenda.horario efetivo)
-- ---------------------------------------------------------------------
-- Sempre que uma sessão é (re)agendada, ativada ou remarcada:
--   1) O horário efetivo é gravado/atualizado em agenda_tratamentos_assistido.horario
--   2) plano_tratamento_sessoes.horario_previsto passa a espelhar ESSE mesmo
--      horário efetivo, na MESMA transação.
-- Assim o "previsto" nunca fica defasado em relação ao "efetivo": não há
-- divergência silenciosa entre plano e agenda. Para não holístico/legado o
-- valor pode ser NULL em ambos (sem divergência). Holístico exige horário.
-- =====================================================================

-- 1) Gravação do plano / sessão ativa
...
DECLARE
  ...
  v_efetivo time without time zone;
BEGIN
  ...
  IF v_tem_ativa THEN
    ...
    IF v_sessao_id IS NULL THEN
      ...
      WHERE assistido_tratamento_id = p_vinculo_id
        AND data_sessao = (p_sessao_ativa->>'data')::date
        AND status = 'agendado' ORDER BY created_at DESC LIMIT 1;
    ELSE
      -- SYNC: atualiza o horário efetivo da sessão existente quando informado
      UPDATE agenda_tratamentos_assistido
        SET horario = COALESCE(v_horario, horario), updated_at = now()
    ...
    UPDATE plano_tratamento_sessoes
      SET status_etapa = 'ativa', agenda_sessao_id = v_sessao_id,
          ...
          horario_previsto = v_efetivo, -- SYNC: previsto espelha o efetivo
          updated_at = now()
...
-- 2) Presença: ativa próxima sessão sincronizando previsto = efetivo
...
BEGIN
  ...
  ELSIF p_proxima_numero_etapa IS NOT NULL AND p_proxima_data IS NOT NULL THEN
    ...
    IF v_sessao_id IS NULL THEN
      ...
      RETURNING horario INTO v_efetivo;
      SELECT id INTO v_sessao_id FROM agenda_tratamentos_assistido
      WHERE assistido_tratamento_id = p_vinculo_id AND data_sessao = p_proxima_data AND status = 'agendado'
      ORDER BY created_at DESC LIMIT 1;
    ELSE
      -- SYNC: atualiza o horário efetivo da sessão existente quando informado
      UPDATE agenda_tratamentos_assistido
        SET horario = COALESCE(p_proxima_horario, horario), updated_at = now()
      WHERE id = v_sessao_id
      RETURNING horario INTO v_efetivo;
    END IF;
    UPDATE plano_tratamento_sessoes
      SET status_etapa = 'ativa', agenda_sessao_id = v_sessao_id, data_prevista = p_proxima_data,
          horario_previsto = v_efetivo, updated_at = now() -- SYNC: previsto espelha o efetivo
...
-- 3) Ausência/remarcação: remarca sincronizando previsto = efetivo
  ...
  ELSE
    IF v_etapa_ativa.id IS NOT NULL AND p_nova_data IS NOT NULL THEN
      ...
      IF v_sessao_id IS NULL THEN
        ...
        RETURNING horario INTO v_efetivo;
        SELECT id INTO v_sessao_id FROM agenda_tratamentos_assistido
        WHERE assistido_tratamento_id = p_vinculo_id AND data_sessao = p_nova_data AND status = 'agendado'
        ORDER BY created_at DESC LIMIT 1;
      ELSE
        -- SYNC: atualiza o horário efetivo da sessão existente quando informado
        UPDATE agenda_tratamentos_assistido
          SET horario = COALESCE(p_nova_horario, horario), updated_at = now()
        WHERE id = v_sessao_id
        RETURNING horario INTO v_efetivo;
      END IF;
      UPDATE plano_tratamento_sessoes
