...
BEGIN
  ...
  WITH sess AS (
    ...
    -- Uma linha legada substituída/materializada por uma etapa do plano
    -- (referenciada via plano_tratamento_sessoes.agenda_sessao_id) NÃO é
    -- contada pelo lado legado; ela é contada apenas pelo plano.
    SELECT t.tarefeiro_id,
      COUNT(*) AS sessoes,
      COUNT(DISTINCT u.assistido_id) AS assistidos
    FROM (
      -- Legado não substituído por plano
