CREATE OR REPLACE FUNCTION public.decidir_promocao_admin(p_request_id uuid, p_decision text, p_motivo text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- The restrictions below apply ONLY to approvals. Rejecting (cancelling) a
  -- request is always safe and must be permitted to any active admin, including
  -- the requester and the indicated user — otherwise a request can deadlock with
  -- no way to cancel it.
  IF p_decision = 'aprovar' THEN
    ...
    -- permanent deadlock. In that single-admin scenario only, the sole
    -- administrator may approve their own request.
    ...
    IF v_req.required_approvals = 1 AND NOT public.is_active_master(v_caller) THEN
