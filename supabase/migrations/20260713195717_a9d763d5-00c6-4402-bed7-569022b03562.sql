-- STAB10-C1.2-A · fix: coluna `instituicao_id` ambígua em
-- fn_autocadastro_assistido_publico (o RETURNS TABLE define a coluna
-- de saída `instituicao_id`, o que colide com `assistidos.instituicao_id`
-- e `instituicao_usuarios.instituicao_id` nas checagens EXISTS).
-- Fix: `#variable_conflict use_column` faz o parser preferir a coluna
-- da tabela em conflitos, mantendo os parâmetros `p_*` sempre distintos.
-- Nenhuma outra mudança de comportamento, segurança ou contrato.
